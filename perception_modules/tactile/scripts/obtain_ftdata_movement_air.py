...
def move_to_pos(robot,pos_command,step, rot_step):
        ...
        if pos_command == "None":
            ...
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[0.0,0.0,0.0174533*8],move_confirm=True)
        elif pos_command == "RotZ-":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[0.0,0.0,-0.0174533*8],move_confirm=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        sawyer.move_to_cartesian_absolute(pos_no = 5)
	
        #Wait 1 second
        time.sleep(1)

	    #Set speed (linear speed m/s, rotational speed rad/s)
        sawyer.set_speed(max_linear_speed = 0.1, max_linear_accel = 0.1, max_rotational_speed = 0.1,max_rotational_accel = 0.1)
        ...
        for movement in test_movements:
            ...
            if direction == "None":
                ...
                sawyer.move_to_cartesian_relative(position=[-0.06,0.0,0.0], orientation=[0.0,0.0,0.0])
                sawyer.move_to_cartesian_absolute(pos_no=6)
            elif direction == "X-":
                sawyer.move_to_cartesian_relative(position=[0.06,0.0,0.0], orientation=[0.0,0.0,0.0])
                sawyer.move_to_cartesian_absolute(pos_no=7)
            elif direction == "Y+":
                sawyer.move_to_cartesian_relative(position=[0.0,-0.06,0.0], orientation=[0.0,0.0,0.0])
                sawyer.move_to_cartesian_absolute(pos_no=8)
            elif direction == "Y-":
                sawyer.move_to_cartesian_relative(position=[0.0,0.06,0.0], orientation=[0.0,0.0,0.0])
                sawyer.move_to_cartesian_absolute(pos_no=9)
            elif direction == "Z+":
                sawyer.move_to_cartesian_absolute(pos_no=10)
            elif direction == "Z-":
                sawyer.move_to_cartesian_absolute(pos_no=10)
                
            #Wait for movement stabilization
            time.sleep(2.0)
            ...
            sawyer.move_to_cartesian_relative(position=[0.0,0.0,0.03], orientation=[0.0,0.0,0.0])
